...
from openlr_services_pb2 import EdgeSet, Edge, Coordinate, RadiusSearchResponse
...
class MapServiceServicer(openlr_services_pb2_grpc.MapServiceServicer):
    ...
    def make_edge(self, e):
        return Edge(
                ...
                frc = e["frc"],
                len = e["len"],
                coords = [Coordinate(longitude=p[0], latitude=p[1]) for p in e["geom"].coords]
            )
    ...
    def find_nearby(self, lon, lat, radius):
        ...
        for e in self.db:
            distance = geod.geometry_length(LineString(nearest_points(e["geom"], point)))
            if (distance <= radius):
    ...
    def RadiusSearch(self, request, context):
        ...
        for p in request.points:
            res.append(EdgeSet(edges=self.find_nearby(float(p.longitude), float(p.latitude), float(request.radius))))
